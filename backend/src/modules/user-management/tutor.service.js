...
export default class TutorService {
  ...
  async handleGetAvailableTutorsForSubject(req, res) {
    try {
      ...
      console.log(
        `Fetching available tutors for subject: ${subjectId}, agency: ${
          agencyId || "any"
        }`
      );
      ...
      const tutors = await this.getAvailableTutorsForSubject(
        subjectId,
        agencyId
      );
      console.log(
        `Found ${tutors.length} available tutors for subject ${subjectId}`
      );
      res.status(200).json({
        success: true,
        data: tutors,
  ...
  async handleCreateTutor(req, res) {
    ...
    await createAndEmailVerificationLink({
      user: newTutor,
  ...
  async getTutors(options = {}) {
    ...
    const where = {
      role: "tutor",
      isSuspended: false, // Filter out suspended tutors
    };
  ...
  async getAvailableTutorsForSubject(subjectId, agencyId = null) {
    console.log(
      `Finding available tutors for subject ${subjectId}, agency: ${
        agencyId || "any"
      }`
    );
    const where = {
    ...
    try {
      ...
      if (subjects !== null) {
        ...
        const subjectsToRemove = currentSubjectIds.filter(
          (id) => !subjects.some((subject) => subject.subjectId === id)
        );

        // subjects to add
        const subjectsToAdd = subjects.filter(
          (subject) => !currentSubjectIds.includes(subject.subjectId)
        );
        ...
        for (const subject of subjectsToUpdate) {
          ...
          if (
            subject.hourlyRate !== undefined &&
            subject.hourlyRate !== tutorSubject.hourlyRate
          ) {
          ...
          if (
            subject.experienceLevel !== undefined &&
            subject.experienceLevel !== tutorSubject.experienceLevel
          ) {
  ...
  async updateTutorSubjectExperience(
    tutorId,
    subjectId,
    experienceLevel,
    transaction = null
  ...
  async handleMarkAttendance(req, res) {
    ...
    console.log(
      `Tutor ${tutorId} attempting to mark attendance ${attendanceId} for lesson ${lessonId}`
    );
  ...
  async getLessonAttendanceData(lesson, now = new Date()) {
    ...
    for (const attendanceRecord of attendanceRecords) {
      const session = this.formatAttendanceSession(
        attendanceRecord,
        lesson,
        now
      );
