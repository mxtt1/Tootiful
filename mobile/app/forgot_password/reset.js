...
export default function ResetPasswordScreen() {
  ...
  const resetToken = decodeURIComponent(String(token || "")); // ← carry token from verify
  ...
  const onSubmit = async () => {
    ...
    if (!resetToken) {
      setErr("Missing reset token. Please verify the code again.");
      return;
    }
    ...
    try {
      setLoading(true);
      // send the token we got from verify
  ...
  return (
    <View style={styles.container}>
      ...
      <Pressable onPress={onSubmit} style={({ pressed }) => [styles.button, pressed && { opacity: 0.9 }]} disabled={loading}>
